...
import static com.example.stripedemo.ViewUtils.getThemeAccentColor;
import static com.example.stripedemo.ViewUtils.getThemeColorControlNormal;
import static com.example.stripedemo.ViewUtils.getThemeTextColorSecondary;
 ...
 * "colorAccent" and "colorControlNormal" states.<p>
 * Author :cgy
 * Date :2019/5/5
 */
public class CreditCardView extends LinearLayout {
    ...
    static {
        ...
        TEMPLATE_RESOURCE_MAP.put(Card.UNIONPAY, com.stripe.android.R.drawable.ic_unionpay_template_32);
    ...
    void init() {
        inflate(getContext(), com.stripe.android.R.layout.masked_card_view, this);
        setOrientation(HORIZONTAL);
        setMinimumWidth(getResources().getDimensionPixelSize(com.stripe.android.R.dimen.card_widget_min_width));
        int paddingPixels = getContext()
                .getResources().getDimensionPixelSize(com.stripe.android.R.dimen.masked_card_vertical_padding);
        setPadding(0, paddingPixels, 0, paddingPixels);

        mCardIconImageView = findViewById(com.stripe.android.R.id.masked_icon_view);
        mCardInformationTextView = findViewById(com.stripe.android.R.id.masked_card_info_view);
        mCheckMarkImageView = findViewById(com.stripe.android.R.id.masked_check_icon);
    ...
    private void initializeCheckMark() {
        updateDrawable(com.stripe.android.R.drawable.ic_checkmark, mCheckMarkImageView, true);
    }
    ...
    private void updateCardInformation() {
        String brandText = Card.AMERICAN_EXPRESS.equals(mCardBrand)
                ? getResources().getString(com.stripe.android.R.string.amex_short)
                : mCardBrand;
        String normalText = getResources().getString(com.stripe.android.R.string.ending_in);
    ...
    private void useDefaultColorsIfThemeColorsAreInvisible() {
        mSelectedColorInt = ViewUtils.isColorTransparent(mSelectedColorInt) ?
                ContextCompat.getColor(getContext(), com.stripe.android.R.color.accent_color_default) :
                mSelectedColorInt;
        mUnselectedColorInt = ViewUtils.isColorTransparent(mUnselectedColorInt) ?
                ContextCompat.getColor(getContext(), com.stripe.android.R.color.control_normal_color_default) :
                mUnselectedColorInt;
        mUnselectedTextColorInt = ViewUtils.isColorTransparent(mUnselectedTextColorInt) ?
                ContextCompat.getColor(getContext(), com.stripe.android.R.color.color_text_secondary_default) :
    ...
    private void setLightTextColorValues() {
        mSelectedAlphaColorInt = ColorUtils.setAlphaComponent(mSelectedColorInt,
                getResources().getInteger(com.stripe.android.R.integer.light_text_alpha_hex));
